...
namespace MemorialRecord.Data
{
    ...
    [Serializable]
    public class PlayerProfile : ISerializationCallbackReceiver
    {
        ...
        public List<bool> watchedStories = new List<bool>();
        public List<bool> unlockedStories = new List<bool>();


        [SerializeField] private bool[] watchedStoriesMeta;
        [SerializeField] private bool[] unlockedStoriesMeta;

        public void OnAfterDeserialize()
        {
            if (watchedStoriesMeta != null)
                watchedStories = new List<bool>(watchedStoriesMeta);

            if (unlockedStoriesMeta != null)
                unlockedStories = new List<bool>(unlockedStoriesMeta);
        }

        public void OnBeforeSerialize()
        {
            watchedStoriesMeta = new bool[watchedStories.Count];
            unlockedStoriesMeta = new bool[unlockedStories.Count];

            watchedStories.CopyTo(watchedStoriesMeta);
            unlockedStories.CopyTo(unlockedStoriesMeta);
        }
    }
    ...
    public class SaveData : ISerializationCallbackReceiver
    {
        ...
        public void ArrayToDict<TKey, TValue>(Dictionary<TKey, TValue> dst, TKey[] keySrc, TValue[] valueSrc)
        {
